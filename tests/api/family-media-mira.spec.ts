...
import { FAMILY_MEDIA_API_PATH, FAMILY_MEMBERS, uniqueMiraMedia } from "../helpers/test-data";

const miraId = FAMILY_MEMBERS.mira.id;
...
async function uploadMiraPhoto(
...
test.describe("Family media API / Mira", () => {
  ...
  test("uploads a photo and returns it in the GET list", async ({ playwright, baseURL }) => {
    ...
    try {
      ...
      await test.step("GET /api/family/media", async () => {
        ...
        expect(
          list.mira.some((entry) => entry.id === created.id && entry.title === media.title),
        ).toBeTruthy();
      });
    } finally {
      if (mediaId) {
    ...
    await test.step("DELETE /api/family/media", async () => {
      await expect(async () => {
        const deleteResponse = await request.delete(FAMILY_MEDIA_API_PATH, {
          data: { memberId: miraId, mediaId: created.id },
        });

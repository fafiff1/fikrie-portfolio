...
import { FAMILY_MEDIA_API_PATH, FAMILY_MEMBERS, uniqueMikhailMedia } from "../helpers/test-data";

const mikhailId = FAMILY_MEMBERS.mikhail.id;
...
async function uploadMikhailPhoto(
...
async function deleteRafaelMedia(request: APIRequestContext, mediaId: string): Promise<void> {
  await expect(async () => {
    const response = await request.delete(FAMILY_MEDIA_API_PATH, {
      data: { memberId: mikhailId, mediaId },
    });
  ...
  test("uploads a photo and returns it in the GET list", async ({ playwright, baseURL }) => {
    ...
    try {
      ...
      await test.step("GET /api/family/media", async () => {
        ...
        expect(
          list.mikhail.some((entry) => entry.id === created.id && entry.title === media.title),
        ).toBeTruthy();
      });
    } finally {
      if (mediaId) {
    ...
    await test.step("DELETE /api/family/media", async () => {
      await expect(async () => {
        const deleteResponse = await request.delete(FAMILY_MEDIA_API_PATH, {
          data: { memberId: mikhailId, mediaId: created.id },
        });

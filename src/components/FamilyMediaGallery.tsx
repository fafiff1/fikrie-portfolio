...
import type { MediaItem } from "@/lib/family-media";

export type { MediaItem };

type FamilyMediaGalleryProps = {
  memberId: string;
  memberName: string;
  media: MediaItem[];
  isLoggedIn: boolean;
...
export default function FamilyMediaGallery({
  memberId,
  memberName,
  media,
  isLoggedIn,
  onMediaChange,
}: FamilyMediaGalleryProps) {
  ...
  const handleDelete = async (item: MediaItem, event: React.MouseEvent) => {
    ...
    try {
      const response = await fetch("/api/family/media", {
        ...
        body: JSON.stringify({ memberId, mediaId: item.id }),
      });
    ...
    <div className="mt-10 pt-8 border-t border-surface-border">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Moments and memories from {memberName}&apos;s adventures
          </p>
        </div>
          ...
          {isLoggedIn && (
            <>
              ...
              <button
                ...
                disabled={uploading}
                className="px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-hover transition-colors flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {uploading ? (
                  <>
            ...
            >
              ...
              >
                ...
                {item.type === "video" && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="w-12 h-12 rounded-full bg-primary/90 flex items-center justify-center text-white group-hover:scale-110 transition-transform">
              ...
              {isLoggedIn && (
                <button
                  ...
                  className="absolute top-3 right-3 w-9 h-9 rounded-full bg-black/80 border border-surface-border flex items-center justify-center text-gray-300 hover:bg-primary hover:text-white hover:border-primary transition-colors z-10 disabled:opacity-60"
      ...
      {filteredMedia.length === 0 && (
        <div className="text-center py-12 text-gray-500 border border-dashed border-surface-border rounded-xl">
          ...
          {isLoggedIn && " Use the Upload button to add some."}
        </div>
      )}

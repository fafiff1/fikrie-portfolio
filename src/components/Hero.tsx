      ...
      <div className="container mx-auto px-6">
        ...
        >
          {content.metrics.map((metric) => (
            <div
              key={metric.label}
              className="border-l border-surface-border pl-4"
            >
              <dt className="text-sm text-gray-400">{metric.label}</dt>
              <dd className="text-3xl md:text-4xl font-bold text-white mt-1">{metric.value}</dd>
            </div>
          ))}
        </motion.dl>
      </div>
